...
function normalizeUser(authUser, authPayload = {}) {
  ...
  return {
    ...
    nome:
      ...
      'Usuario',
    fotoUrl: collaborator?.foto_url || null,
...
function mapAuthError(error) {
  ...
  if (error?.status === 401) {
    return {
      type: 'auth_required',
      message: error.message || 'Faca login para acessar a Comunicacao.',
    };
  }
  ...
  if (error?.status === 403) {
    return {
      type: 'access_denied',
      message: error.message || 'Seu usuario nao possui acesso liberado a Comunicacao.',
    };
  }

  return {
    type: 'unknown',
    message: error?.message || 'Nao foi possivel validar o acesso a Comunicacao.',
  };
}
...
export function AuthProvider({ children }) {
  ...
  async function runAccessValidation(nextSession) {
    ...
    try {
      const authPayload = await comunicacaoApi.auth.me(nextSession.access_token);
      const currentUser = normalizeUser(nextSession.user, authPayload);

      if (!currentUser.active) {
        const accessError = new Error('Seu usuario nao possui acesso ativo a Comunicacao.');
  ...
  async function login(email, password, captchaToken) {
    assertSupabaseConfigured();
    const lock = await checkLoginLock(email, 'comunicacao');
    ...
    if (error) {
      reportFailedLogin(email, 'comunicacao');
      throw error;
    }
    reportLoginSuccess('comunicacao');
