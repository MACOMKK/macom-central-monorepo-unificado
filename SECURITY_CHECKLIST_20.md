...
1. ~~**#20 — Scan de dependências**~~ — feito em 2026-08-27: script `npm run audit` (`npm audit --audit-level=high`) adicionado ao `package.json` raiz. `npm audit fix` (sem `--force`) reduziu de 23 para 10. As 4 pendências restantes (react-quill, @capacitor/cli, vite-plugin-pwa/sharp, react-router-dom) foram tratadas em 3 etapas isoladas e testadas: Etapa 1 (`@capacitor/cli`) sem ação necessária, já na versão estável mais recente; Etapa 2 (`sharp` via `overrides`) 10→7; Etapa 3 (`react-router-dom` v6→v7.18.2, maior risco por tocar todos os apps) 7→5, validada com testes automatizados, typecheck, build dos 6 apps e navegação manual. Restam 5 vulnerabilidades (react-quill, uuid/xcode em tooling iOS não usado), ambas sem correção disponível que não seja regressiva — documentadas como risco aceito. Ainda não configurado Dependabot/CI automático.
